## ----------------------------------------------------------------
## [Jupyter용] 2_Train_Models_v19_agegroup_yme.py
##  - v18 베이스
##  - 전략 2: PK 히스토리 피처
##  - 전략 3: Age/YearMonthIndex 정규화 점수
##  - [변경] *_cost → *_log_ratio (로그 비율)
##  - [추가] Delta 피처: PK 기준 직전 관측과의 차이 (B만 사용)
##  - [추가] AgeGroup / Age_num_z2 / Age_num_z3
##  - [추가] YM_is_early (초기 고위험 시점 플래그)
...
print("Big-Tech ML Engineer (v19+age_group+ym_early): K-Fold + Leakage Fix + Domain Features 시작.")
...
try:
    ...
    print(f"경고: {e}")
    print("먼저 1_Preprocess_v19_agegroup_yme.py를 실행하여 all_train_data.feather를 생성해야 합니다.")
    raise
print("데이터 로드 완료.")
...
def _build_cb_matrices(X_train, X_val):
    ...
    # ⚠️ 여기서 Categorical → object/str로 변환 후 fillna
    for col in CAT_FEATURES:
        if col in cb_X_train.columns:
            # 먼저 object로 캐스팅해서 Categorical 제약 제거
            train_col = cb_X_train[col].astype("object")
            val_col   = cb_X_val[col].astype("object")

            # 그 다음 결측을 'nan' 문자열로 채우고 str로 변환
...
def train_model_A(X_train, y_train, X_val, y_val, group_label="A"):
    ...
    A 모델: PK Stats 사용 안 함
      ...
      + AgeGroup, Age_num_z2, Age_num_z3, YM_is_early 등
      + log_ratio 기반 도메인 피처
      + Delta 피처는 명시적으로 제거
    """
    ...
    print(f"\n[{group_label}] CatBoost (Base+Hist+Norm+log_ratio+AgeGroup+YM_early, no PK, no Delta) 학습 시작... (피처 {len(cb_X_train.columns)}개)")
...
def train_model_B(X_train, y_train, X_val, y_val, pk_stats_fold, group_label="B"):
    """
    B 모델: PK Stats + Delta(B-only) + log_ratio + AgeGroup + YM_early 모두 사용
    """
    # PK Stats merge
    ...
    print(f"[{group_label}] CatBoost (Base+PK+Hist+Norm+Delta+log_ratio+AgeGroup+YM_early) 학습 시작... (피처 {len(cb_X_train.columns)}개)")
    cat_base_model = cb.CatBoostClassifier(
        iterations=3000,
        loss_function='Logloss',
        eval_metric='AUC',
...
for fold in range(N_SPLITS):
    ...
    # --- 모델 A 학습 ---
    print("\n--- 모델 A (PK Stats 미사용, Delta 제외, log_ratio + AgeGroup + YM_early 포함) 학습 ---")
    ...
    # --- 모델 B 학습 ---
    print("\n--- 모델 B (PK Stats + Delta + log_ratio + AgeGroup + YM_early, tuned) 학습 ---")
...
print("Big-Tech ML Engineer (v19+age_group+ym_early): 미션 완료.")
