...
# ---- GPU/CPU 선택 옵션 (환경변수) ----
CAT_TASK_TYPE = os.getenv("CAT_TASK_TYPE", "CPU")   # "GPU" or "CPU"
CAT_DEVICES   = os.getenv("CAT_DEVICES", "0")       # "0", "0-1" 등
print(f"[INFO] CatBoost task_type={CAT_TASK_TYPE}, devices={CAT_DEVICES}")
...
## 4. 공통 CatBoost 입력 구성 함수 (컬럼 순서 보존)
def _build_cb_matrices(X_train, X_val):
    ...
    """
    # 원본 순서 보존
    numeric_cols = [c for c in X_train.columns if c not in CAT_FEATURES + DROP_COLS_TRAIN]
    # 공통 교집합(순서 보존)
    common_numeric_cols = [c for c in numeric_cols if c in X_val.columns]
...
print("Big-Tech ML Engineer (v21): 미션 완료.")
