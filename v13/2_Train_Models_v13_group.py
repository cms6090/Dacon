## ----------------------------------------------------------------
## 2_Train_Models_v13_group.py
##  - StratifiedGroupKFold(PrimaryKey) 교차 검증
##  - Model A: v11 기본 CatBoost (AUC 기준)
##  - Model B: 튜닝된 CatBoost (Logloss 기준, Balanced class)
##  - PK Stats + OOF Isotonic (Fold별 + Global)
## ----------------------------------------------------------------
...
print("Big-Tech ML Engineer v13(group): GroupKFold + Tuned-B CatBoost + OOF Isotonic 전략 시작.")
...
# ★ StratifiedGroupKFold: 라벨 분포 + 운전자(PrimaryKey) 그룹 동시 고려
...
def train_model_B(X_train, y_train, X_val, y_val, pk_stats_fold, group_label="B"):
    """
    자격유지검사(B)용 모델.
    - Fold-train 기준 PK Stats 병합
    - 튜닝된 CatBoost 세팅 (Logloss 기준, Balanced class)
    """
    # PK Stats (fold train 기준으로만 생성한 것 병합 → leakage 방지)
...
print("Big-Tech ML Engineer v13(group): 미션 완료.")
